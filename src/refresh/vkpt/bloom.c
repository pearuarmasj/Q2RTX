...
{
#ifdef CONFIG_USE_DLSS
	// Use unscaled height for DLSS, TAA output height otherwise
	int output_height = (DLSSEnabled() && DLSSCreated()) 
		? qvk.extent_unscaled.height 
		: qvk.extent_taa_output.height;
#else
	int output_height = qvk.extent_taa_output.height;
#endif
	float sigma_pixels = bloom_sigma * (float)output_height;
...
{
#ifdef CONFIG_USE_DLSS
	qboolean use_dlss = DLSSEnabled() && DLSSCreated();
	VkExtent2D extent = use_dlss ? qvk.extent_unscaled : qvk.extent_taa_output;
#else
