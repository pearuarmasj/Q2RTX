...
#define UBO_CVAR_LIST \
	...
	UBO_CVAR_DO(pt_restir_enable, 0) /* enables experimental ReSTIR DI in direct lighting pass, 0 or 1 */ \
	UBO_CVAR_DO(pt_restir_temporal, 0) /* enables experimental ReSTIR DI temporal reuse, 0 or 1 */ \
	UBO_CVAR_DO(pt_restir_spatial, 0) /* enables experimental ReSTIR DI spatial reuse (neighbor taps), 0 or 1 */ \
	UBO_CVAR_DO(pt_restir_spatial_neighbors, 4) /* number of neighbor reservoir taps for spatial reuse, [0..16] */ \
	UBO_CVAR_DO(pt_restir_candidates, 8) /* number of per-pixel ReSTIR DI candidates (single-frame MVP), [1..32] */ \
	UBO_CVAR_DO(pt_restir_max_w, 3.0) /* ReSTIR DI max weight clamp, limits fireflies, (0..inf) */ \
	UBO_CVAR_DO(pt_restir_debug_view, 0) /* ReSTIR DI debug view selector, 0 disables */ \
	UBO_CVAR_DO(pt_restir_gi_enable, 0) /* enables ReSTIR GI for indirect lighting, 0 or 1 */ \
	UBO_CVAR_DO(pt_restir_gi_temporal, 1) /* enables ReSTIR GI temporal reuse, 0 or 1 */ \
	UBO_CVAR_DO(pt_restir_gi_spatial, 1) /* enables ReSTIR GI spatial reuse, 0 or 1 */ \
	UBO_CVAR_DO(pt_restir_gi_spatial_neighbors, 4) /* number of spatial neighbor taps for GI, [1..16] */ \
	UBO_CVAR_DO(pt_restir_gi_max_history, 20) /* maximum temporal history length for GI reservoirs, [1..255] */ \
	UBO_CVAR_DO(pt_restir_gi_max_reservoir_age, 30) /* maximum age for GI reservoirs before discard, [1..255] */ \
	UBO_CVAR_DO(pt_restir_gi_depth_threshold, 0.1) /* depth similarity threshold for GI reuse, [0..1] */ \
	UBO_CVAR_DO(pt_restir_gi_normal_threshold, 0.5) /* normal similarity threshold for GI reuse (dot product), [0..1] */ \
	UBO_CVAR_DO(pt_restir_gi_spatial_radius, 32.0) /* spatial sampling radius in pixels for GI, [1..64] */ \
	UBO_CVAR_DO(pt_restir_gi_debug_view, 0) /* ReSTIR GI debug view selector, 0 disables */ \
	UBO_CVAR_DO(pt_dlss, 0) /* DLSS upscaling mode: 0 disabled, 1+ enabled quality mode */ \
	UBO_CVAR_DO(pt_dlssdn, 0) /* DLSS Ray Reconstruction (denoising) mode: 0 disabled, 1 enabled */ \
	UBO_CVAR_DO(pt_rng_seed, -1) /* overrides RNG frame index when >= 0, enables reproducible captures */ \
