...
// ReSTIR DI stores per-pixel reservoir history in ping-pong images. When the view, resolution,
// or key rendering settings change, that history becomes invalid and must be cleared to avoid
// reusing stale/uninitialized data.
static bool restir_di_history_clear_pending = true;

static void vkpt_restir_di_invalidate_history(void)
{
	restir_di_history_clear_pending = true;
}
...
VkptInit_t vkpt_initialization[] = {
	...
	{ "restir_gi", vkpt_restir_gi_initialize,          vkpt_restir_gi_destroy,               VKPT_INIT_DEFAULT,            0 },
	{ "restir_gi|", vkpt_restir_gi_create_pipelines,   vkpt_restir_gi_destroy_pipelines,     VKPT_INIT_RELOAD_SHADER,      0 },
...
{
	// ReSTIR reuse depends on stable history.
	num_accumulated_frames = 0;
	vkpt_restir_di_invalidate_history();
}

static void vkpt_restir_di_clear_history_images(VkCommandBuffer cmd_buf)
{
	// Images are created during VKPT_INIT_SWAPCHAIN_RECREATE. If we're minimized,
	// or images were not initialized yet, keep the pending flag and retry later.
	if (!qvk.images[VKPT_IMG_PT_RESTIR_DI_ID_A])
		return;
	...
	const VkClearColorValue clear_zero = { 0 };

	const int restir_images[] = {
		VKPT_IMG_PT_RESTIR_DI_ID_A,
		VKPT_IMG_PT_RESTIR_DI_ID_B,
		VKPT_IMG_PT_RESTIR_DI_RES_A,
		VKPT_IMG_PT_RESTIR_DI_RES_B,
	};

	for (int i = 0; i < LENGTH(restir_images); i++)
	{
		const int img_idx = restir_images[i];
		VkImage img = qvk.images[img_idx];
		...
		vkCmdClearColorImage(cmd_buf, img, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clear_zero, 1, &subresource_range);
...
#ifdef CONFIG_USE_DLSS
// Clear Ray Reconstruction PT_ source buffers at frame start
static void vkpt_dlss_rr_clear_buffers(VkCommandBuffer cmd_buf)
{
	// Only clear if DLSS Ray Reconstruction is enabled
	...
	const int rr_pt_images[] = {
		VKPT_IMG_PT_ALBEDO,
		VKPT_IMG_PT_SPECULAR,
		VKPT_IMG_PT_ROUGHNESS,
		VKPT_IMG_PT_METALLIC_RR,
		VKPT_IMG_PT_NORMAL_RR,
		VKPT_IMG_PT_MATERIALID,
		VKPT_IMG_PT_EMISSIVE_RR,
		...
		VKPT_IMG_PT_COMPOSITE_TRANSPARENT,
		VKPT_IMG_PT_VIEW_DEPTH_SPLIT,
	};

	for (int i = 0; i < LENGTH(rr_pt_images); i++)
	{
		const int img_idx = rr_pt_images[i];
		VkImage img = qvk.images[img_idx];
...
// Copy PT_ buffers to DLSS_ buffers before DLSS-RR evaluation
static void vkpt_dlss_rr_copy_buffers(VkCommandBuffer cmd_buf)
{
	// Only copy if DLSS Ray Reconstruction is enabled
	if (!DLSSEnabled() || DLSSModeDenoise() != 1)
		return;
	...
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_ALBEDO, VKPT_IMG_DLSS_ALBEDO, zero_offset, zero_offset, render_extent);
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_SPECULAR, VKPT_IMG_DLSS_SPECULAR, zero_offset, zero_offset, render_extent);
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_ROUGHNESS, VKPT_IMG_DLSS_ROUGHNESS, zero_offset, zero_offset, render_extent);
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_METALLIC_RR, VKPT_IMG_DLSS_METALLIC, zero_offset, zero_offset, render_extent);
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_NORMAL_RR, VKPT_IMG_DLSS_NORMAL, zero_offset, zero_offset, render_extent);
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_MATERIALID, VKPT_IMG_DLSS_MATERIALID, zero_offset, zero_offset, render_extent);
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_EMISSIVE_RR, VKPT_IMG_DLSS_EMISSIVE, zero_offset, zero_offset, render_extent);
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_INDIRECT_ALBEDO, VKPT_IMG_DLSS_INDIRECT_ALBEDO, zero_offset, zero_offset, render_extent);
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_SPECULAR_ALBEDO, VKPT_IMG_DLSS_SPECULAR_ALBEDO, zero_offset, zero_offset, render_extent);
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_BEFORE_TRANSPARENT, VKPT_IMG_DLSS_BEFORE_TRANSPARENT, zero_offset, zero_offset, render_extent);
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_RAYLENGTH_DIFFUSE, VKPT_IMG_DLSS_RAYLENGTH_DIFFUSE, zero_offset, zero_offset, render_extent);
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_RAYLENGTH_SPECULAR, VKPT_IMG_DLSS_RAYLENGTH_SPECULAR, zero_offset, zero_offset, render_extent);
	vkpt_image_copy(cmd_buf, VKPT_IMG_PT_REFLECTED_ALBEDO, VKPT_IMG_DLSS_REFLECTED_ALBEDO, zero_offset, zero_offset, render_extent);
}
#endif
...
#ifdef CONFIG_USE_DLSS
	// Apply DLSS resolution scaling if enabled
	if (DLSSEnabled())
	{
		float dlss_scale = GetDLSSResolutionScale();
		result.width = (uint32_t)(qvk.extent_unscaled.width * dlss_scale);
		result.height = (uint32_t)(qvk.extent_unscaled.height * dlss_scale);
	}
#endif
...
evaluate_taa_settings(const reference_mode_t* ref_mode)
{
	...
	// When DLSS is enabled, disable TAA - DLSS handles temporal component
	if (dlssEnabled) {
		flt_taa = AA_MODE_OFF;
	}
#else
...
R_RenderFrame_RTX(refdef_t *fd)
{
...

	{
		...
		// Clear RR buffers at frame start (before any path tracing writes)
		if (DLSSEnabled() && DLSSModeDenoise() == 1)
		{
			vkpt_dlss_rr_clear_buffers(trace_cmd_buf);
		}
...

	{
		...
		// ReSTIR GI: temporal/spatial resampling of indirect lighting
		// This runs after bounce ray tracing and before the denoiser
		BEGIN_PERF_MARKER(trace_cmd_buf, PROFILER_ASVGF_FULL);  // Temporary - needs own marker
		vkpt_restir_gi_record_cmd_buffer(trace_cmd_buf);
		END_PERF_MARKER(trace_cmd_buf, PROFILER_ASVGF_FULL);
...
#ifdef CONFIG_USE_DLSS
		// When DLSS is disabled, run bloom here before upscaling
		if (!DLSSEnabled()) {
#endif
...
#ifdef CONFIG_USE_DLSS
		// When DLSS is enabled, run DLSS BEFORE tone mapping
		// DLSS reads HDR data and outputs to DLSS_OUTPUT, then tone mapping processes that
		if (DLSSEnabled())
		{
...
#ifdef CONFIG_USE_DLSS
		// When DLSS is enabled, run bloom AFTER tone mapping (for menu blur at full resolution)
		if (DLSSEnabled())
		{
			BEGIN_PERF_MARKER(post_cmd_buf, PROFILER_BLOOM);
			if (cvar_bloom_enable->integer != 0 || qvk.frame_menu_mode)
...
recreate_swapchain(void)
{
	...
	// Any swapchain recreation invalidates temporal history, and also changes image extents.
	// Reset the "previous" extents to prevent out-of-bounds history fetches on the first frame.
	temporal_frame_valid = false;
	qvk.extent_render_prev = qvk.extent_render;
	qvk.gpu_slice_width_prev = qvk.gpu_slice_width;
	vkpt_restir_di_invalidate_history();
...
retry:;
	...
	// Process the profiler queries - always enabled to support DRS
	{
		...
		if (restir_di_history_clear_pending && qvk.images[VKPT_IMG_PT_RESTIR_DI_ID_A])
		{
			vkpt_restir_di_clear_history_images(reset_cmd_buf);
			restir_di_history_clear_pending = false;
		}
...
#ifdef CONFIG_USE_DLSS
		// If DLSS is enabled and active, use DLSS output (includes menu blur when in menu mode)
		if (DLSSEnabled() && DLSSCreated())
		{
			vkpt_final_blit(cmd_buf, VKPT_IMG_DLSS_OUTPUT, qvk.extent_unscaled, false, qvk.frame_menu_mode ? false : waterwarp);
		}
		else
#endif
		if (vkpt_fsr_is_enabled() && !qvk.frame_menu_mode)
...
#undef UBO_CVAR_LIST
	...
	// Any change to ReSTIR parameters should reset its history to avoid mixing reservoirs
	// from different algorithms/settings.
	cvar_pt_restir_enable->changed = restir_cvar_changed;
	cvar_pt_restir_debug_view->changed = restir_cvar_changed;
	cvar_pt_restir_candidates->changed = restir_cvar_changed;
	cvar_pt_restir_max_w->changed = restir_cvar_changed;
	cvar_pt_restir_temporal->changed = restir_cvar_changed;
	cvar_pt_restir_spatial->changed = restir_cvar_changed;
	cvar_pt_restir_spatial_neighbors->changed = restir_cvar_changed;
	cvar_pt_rng_seed->changed = restir_cvar_changed;
...
#ifdef CONFIG_USE_DLSS
	// Initialize DLSS after Vulkan device creation
	// GUID format is required for NVIDIA NGX SDK project identification
	if (!DLSSConstructor(qvk.instance, qvk.device, qvk.physical_device, "231E3AD6-F0B4-4C87-B63E-E8C4FA7B5CC1", qvk.enable_validation)) {
		Com_Printf("DLSS: Not available, falling back to standard upscaling\n");
	}
#endif
