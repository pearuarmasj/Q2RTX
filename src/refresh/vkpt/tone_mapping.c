...
vkpt_tone_mapping_record_cmd_buffer(VkCommandBuffer cmd_buf, float frame_time)
{
	...
	VkExtent2D output_extent = use_dlss ? qvk.extent_unscaled : qvk.extent_taa_output;
	if (use_dlss) {
...
#else
	VkExtent2D output_extent = qvk.extent_taa_output;
	BARRIER_COMPUTE(cmd_buf, qvk.images[VKPT_IMG_TAA_OUTPUT]);
#endif
	...
	// Because output image changed, we make sure to wait for the image
